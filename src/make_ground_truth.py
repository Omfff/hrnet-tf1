...
class GroundTruth(object):
    ...
    def __tensor2list(tensor_data):
        ...
        for i in range(length):
            list_data.append(bytes.decode(tensor_data[i], encoding="utf-8"))
        return list_data
    ...
    def get_ground_truth(self):
        ...
        for item in self.batch_keypoints_list:
            ...
            target, target_weight = self.__generate_target(keypoints_3d.eval(), keypoints_3d_exist.eval())
    ...
    def __get_one_human_instance_keypoints(self, line_keypoints):
        ...
        keypoints_tensor = tf.convert_to_tensor(value=keypoints, dtype=tf.float32)
    ...
    def __get_keypoints_3d(self, keypoints):
        ...
        for i in range(self.num_of_joints):
            keypoints_3d_list.append(tf.convert_to_tensor([keypoints[i, 0], keypoints[i, 1], 0], dtype=tf.float32))
            exist_value = keypoints[i, 2]
            if exist_value > 1:
                exist_value = 1
            # exist_value: (1: exist , 0: not exist)
            keypoints_3d_exist_list.append(tf.convert_to_tensor([exist_value, exist_value, 0], dtype=tf.float32))
    ...
    def __generate_target(self, keypoints_3d, keypoints_3d_exist):
        ...
        target_weight = tf.convert_to_tensor(target_weight, tf.float32)    # shape : (self.num_of_joints, 1)
