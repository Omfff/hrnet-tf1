...
def up_sampling(input, channels, method='nearest_neighbor', rate=2, name='', activation=leaky_Relu, is_training=True):
    ...
    if method == 'nearest_neighbor':
        ...
        x = x.value
        y = y.value
        output = conv_2d(input, channels=channels, kernel_size=1, activation=None,batch_normalization=True,
                         name=name + '_align_channels', is_training=is_training)
        name += '_align_channels'
        output = tf.image.resize_nearest_neighbor(output, size=(x*rate, y*rate), name=name + '_upsampling')
    return output
...
def exchange_between_stage(inputs, input_branch_num, out_branch_num, name='exchange_between_stage', is_training=True):
    with tf.variable_scope(name):
        subnetworks_number = input_branch_num
        outputs = []

        # suppose i is the index of the input sub-network, o is the index of the output sub-network
        # 2
        # o:0 i:0 1
        for o in range(out_branch_num):
            one_subnetwork = 0
            for i in range(subnetworks_number):
                if i == o:
                    # if in the same resolution
...
def exchange_between_stem_and_stage(inputs, input_branch_num, out_branch_num, name='exchange_between_stem_and_stage', is_training=True):
    with tf.variable_scope(name):
        subnetworks_number = input_branch_num
        outputs = []
        ...
        for o in range(out_branch_num):
            ...
            if o == 0:
                # if in the same resolution
                one_subnetwork = conv_2d(inputs[o], channels = 32, kernel_size=3, strides=1, batch_normalization=True, activation=tf.nn.relu,
            name='i_0_o_0', padding='same', is_training=is_training)
            else:
                one_subnetwork = conv_2d(inputs[0], channels= 64, strides=2, activation=tf.nn.relu,
                                 name=name + 'down_sampling' , is_training=is_training)
            outputs.append(one_subnetwork)
        #one_subnetwork = down_sampling(inputs[-1], rate=2, name='new_resolution', is_training=is_training)
        #outputs.append(one_subnetwork)
        return outputs
...
def residual_unit_bottleneck(input, name='RU_bottleneck', channels=64, is_training=True):
    ...
    conv_3x3 = conv_2d(conv_1x1_1, channels=channels, activation=leaky_Relu, name=name + '_conv3x3',
                       is_training=is_training)
    conv_1x1_2 = conv_2d(conv_3x3, channels=channels*4, kernel_size=1, name=name + '_conv1x1_2', is_training=is_training)
    if c != channels*4:
        input_down = conv_2d(input, channels=channels * 4, kernel_size=1, name=name + 'input_downsample',
                             is_training=is_training, strides=1)
    else:
        input_down = input
    _output = tf.add(input_down, conv_1x1_2, name=name + '_add')
