...
from high_resolution_module import *


def stage1(input, name='stage1', is_training=True):
    output = []
    with tf.variable_scope(name):
        scale_conv1 = conv_2d(input, channels = 64, kernel_size=3, strides=2, batch_normalization=True, activation=None,
            name='scale_conv1', padding='same', is_training=is_training)
        scale_conv2 = conv_2d(scale_conv1, channels=64, kernel_size=3, strides=2, batch_normalization=True, activation=None,
                              name='scale_conv2', padding='same', is_training=is_training)
        s1_res1 = residual_unit_bottleneck(scale_conv2, name='rs1', is_training=is_training)
...
def stage2(input, name='stage2', is_training=True):
    with tf.variable_scope(name):
        inputs = []
        inputs.append(input)
        sub_networks = exchange_between_stem_and_stage(inputs,input_branch_num = 1, out_branch_num = 2, name='between_stage', is_training=is_training)
...
def stage3(input, name='stage3', is_training=True):
    with tf.variable_scope(name):
        sub_networks = exchange_between_stage(input,input_branch_num = 2, out_branch_num = 3, name=name, is_training=is_training)
...
def HRNet(input, is_training=True, eps=1e-10):
    ...
    # will make the loss of one image to be around 1.75 (batch_size = 1, 256, 192, 17). This is because that the value
    ...
    # sum up the value on each pixels, the result should be a [batch_size, 17] tensor, then expend dim to be
    # [batch_size, 1, 1, 17] tensor so as to normalize the output
