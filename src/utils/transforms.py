...
def read_image(image_dir, cfg):
    ...
    # image_content = tf.read_file(filename=image_dir)
    # # The 'image' has been normalized.
    # image = tf.image.decode_image(contents=image_content, channels=cfg.CHANNELS)
    #image = tf.cast(img, tf.float32, name='change_type')
    image = tf.convert_to_tensor(img,dtype=tf.float32)
    image = tf.divide(image, 255.0, name=None)
    #print(image)
    return image
...
class RandomCropTransform(object):
    ...
    def keypoints_transform(self, resize_ratio, crop_rect):
        crop_rect = crop_rect.eval()
        transformed_keypoints = self.keypoints.eval()
...
class ResizeTransform(object):
    ...
    def image_transform(self):
        human_instance = tf.image.crop_to_bounding_box(image=self.image,
                                                       ...
                                                       target_width=self.bbox[2])
        left_top_of_human_instance = self.bbox[0:2]
        resize_ratio = [self.resize_h / human_instance.shape[0].value, self.resize_w / human_instance.shape[1].value]
        resized_image = tf.image.resize_images(images=human_instance, size=[self.resize_h, self.resize_w])
