...
class COCO_keypoints(object):
    def __init__(self):
        ...
        self.train_images_dir = Config.COCO_ROOT_DIR + "train2017/"
        self.valid_images_dir = Config.COCO_ROOT_DIR + "valid2017/"
    ...
    def __get_the_path_of_picture(self, picture_name, type):
        if type == "train":
            return self.train_images_dir + picture_name
        else:
            return self.valid_images_dir + picture_name
    ...
    def write_information_to_txt(self, dataset):
        ...
        with open(name=txt_file, mode="a+") as f:
            for i in range(len(image_ids_from_keypoints)):
                one_human_instance_info = ""
                image_index = image_id_dict[image_ids_from_keypoints[i]]
                one_human_instance_info += self.__get_the_path_of_picture(image_files[image_index], dataset) + " "
